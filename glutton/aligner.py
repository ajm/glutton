...
from glutton.utils import tmpfile, num_threads, get_log, rm_f, check_dir
...
class Aligner(object) :
    def __init__(self, db, contigs_fname, directory, min_identity=0.9, min_length=1, batch_size=100) :
        ...
        # else we are restarting
            ...
            if self._not_same_db(db_params) :
                self.log.error("alignment has been resumed with a different reference!")
                self.log.error("\toriginal = %s" % self._param_str(self.params))
                self.log.error("\tcurrent  = %s" % self._param_str(db_params))
                sys.exit(1)
    ...
    def get_params(self) :
        ...
        p['species']  = self.db.species
        p['release']  = self.db.release
        p['filename'] = self.db.filename
        p['checksum'] = self.db.checksum
