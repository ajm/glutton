...
def sga(fasta, k, kmer_threshold, merge_overlap, assemble_overlap) :
    global threads

    out = "assembly.%s.k%d" % (clipext(fasta), k)
    ...
    cleanup(glob("correct.*") + glob("merged.*") + glob(clipext(clipext(fasta)) + ".*"))
